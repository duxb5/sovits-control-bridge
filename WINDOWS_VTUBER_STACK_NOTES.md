...
### Antigravity 에이전트 답변 자동 연동 (Direct Integration)

안티그래비티 에이전트가 답변 시 별도의 요약 과정 없이 답변 전체를 소리 내어 읽게 하려면:

1. 안티그래비티 답변 텍스트 버퍼를 UTF-8로 저장합니다:
   `C:\Users\jun\Documents\VTuber\sovits-output\speech_input.txt`
2. 전용 파이썬 스크립트를 통해 SoVITS Voice Bridge로 전송하여 재생합니다:
   `Get-Content -Raw -Encoding utf8 C:\Users\jun\Documents\VTuber\sovits-output\speech_input.txt | C:\Users\jun\Documents\VTuber\Open-LLM-VTuber\.venv\Scripts\python.exe C:\Users\jun\Documents\VTuber\sovits-send.py`
   *(※ 안티그래비티의 전역 에이전트 규칙인 `AGENTS.md`에 설정되어 자동으로 실행됩니다.)*
